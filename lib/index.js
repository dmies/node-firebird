...
SQLVarDate.prototype.decode = function(data) {
  ...
  if (!data.readInt()) {
    var d = new Date(0);
    d.setMilliseconds(
      (ret - DateOffset) * TimeCoeff + d.getTimezoneOffset() * MsPerMinute
    );
    return d;
  }
  return null;
...
SQLVarTime.prototype.decode = function(data) {
  ...
  if (!data.readInt()) {
    var d = new Date(0);
    d.setMilliseconds(
      Math.floor(ret / 10) + d.getTimezoneOffset() * MsPerMinute
    );
    return d;
  }
  return null;
...
SQLParamDate.prototype.encode = function(data) {
  if (this.value != null) {
    var value =
      this.value.getTime() - this.value.getTimezoneOffset() * MsPerMinute;
        ...
        switch (statement.type) {
          case isc_info_sql_stmt_select:
            statement.fetchAll(self, function(err, ret) {
              ...
              if (callback) callback(undefined, ret, statement.output, true);
            });
...
Transaction.prototype.query = function(query, params, callback) {
  ...
  this.execute(query, params, callback, {
    asObject: true,
...
Database.prototype.sequentially = function(
  query,
  params,
  on,
  callback,
  ...
  if (on === undefined) throw new Error('Expected "on" delegate.');
...
ServiceManager.prototype.__proto__ = Object.create(
...
ServiceManager.prototype._createOutputStream = function(
  optread,
  buffersize,
  callback
) {
  ...
  var t = new stream.Readable({
    objectMode: optread == "byline" ? true : false
  }); // chunk by line
...
ServiceManager.prototype._processcapabilities = function(blr, res) {
  ...
  for (var i = 0; i < capArray.length; ++i)
    if (caps & (1 << i)) dbcapa.push(capArray[i]);
};
...
ServiceManager.prototype.backup = function(options, callback) {
  var dbpath =
    options.database ||
    this.connection.options.filename ||
  ...
  if (bckfiles)
    bckfiles =
      bckfiles.constructor !== Array
        ? [{ filename: bckfiles, sizefile: "0" }]
        : bckfiles;
...
ServiceManager.prototype.setSweepinterval = function(
  db,
  sweepinterval,
  callback
) {
...
ServiceManager.prototype.Shutdown = function(db, kind, delay, mode, callback) {
  ...
  this._fixpropertie(
    { database: db, shutdown: kind, shutdowndelay: delay, shutdownmode: mode },
    callback
  );
};
...
ServiceManager.prototype.commit = function(db, transactid, callback) {
  var dbpath =
    db || this.connection.options.filename || this.connection.options.database;
  if (dbpath == null || dbpath.length == 0) {
    doError(new Error("No database specified"), callback);
    return;
  }
...
ServiceManager.prototype.addUser = function(
  username,
  password,
  options,
  callback
...
ServiceManager.prototype.getFbserverInfos = function(infos, options, callback) {
  ...
  var tops = [];
  for (popts in opts)
    if (infos[popts] || infos.length == 0) tops.push(opts[popts]);
...
ServiceManager.prototype.readline = function(options, callback) {
  ...
  var self = this;
  this.connection.svcquery([isc_info_svc_line], buffersize, timeout, function(
    err,
    data
  ) {
    if (err || !data.buffer) {
...
ServiceManager.prototype.readeof = function(options, callback) {
  ...
  var timeout = options.timeout || 60;
  var self = this;
  this.connection.svcquery([isc_info_svc_to_eof], buffersize, timeout, function(
    err,
    data
  ) {
    if (err || !data.buffer) {
    ...
    exports.attach(self.options, function(err, db) {
      if (!err) {
        ...
        db.on("detach", function() {
          ...
          if (
            self.pooldb.indexOf(db) != -1 ||
            self.internaldb.indexOf(db) == -1
          )
            return;
          // if not usable don't put in again in the pool and remove reference on it
          if (
            db.connection._isClosed ||
            db.connection._isDetach ||
...
Pool.prototype.destroy = function() {
  ...
  this.internaldb.forEach(function(db) {
    if (db.connection._pooled == false) return;
...
exports.Connection.prototype._setcachedquery = function(query, statement) {
  if (this._cache_query)
    if (
      this._max_cached_query == -1 ||
      this._max_cached_query > Object.keys(this._cache_query).length
    )
      this._cache_query[query] = statement;
};

exports.Connection.prototype.getCachedQuery = function(query) {
  if (this._cache_query) return this._cache_query[query];
  return null;
};
...
exports.Connection.prototype._bind_events = function(host, port, callback) {
  ...
  self._socket.on("data", function(data) {
    ...
    while (xdr.pos < xdr.buffer.length) {
      ...
      } else doCallback(obj, cb);
    }

    if (!self._detachAuto || self._pending.length !== 0) return;
...
function decodeResponse(data, callback, db, lowercase_keys) {
  ...
  switch (r) {
    case op_response:
      ...
      if (oid.low || oid.high) response.oid = oid;

      var buf = data.readArray();
      if (buf) response.buffer = buf;
      ...
      while (true) {
        ...
        switch (op) {
          ...
          case isc_arg_gds:
            ...
            if (!num) break;
            item = { gdscode: num };
            if (response.status) response.status.push(item);
            else response.status = [item];
            break;
          case isc_arg_string:
          ...
          case isc_arg_sql_state:
            ...
            } else item.params = [data.readString(DEFAULT_ENCODING)];
          ...
          case isc_arg_number:
            ...
            if (item.params) item.params.push(num);
            else item.params = [num];

            if (item.gdscode === isc_sqlerr) response.sqlcode = num;
    ...
    case op_fetch_response:
      ...
      if (custom.asObject && !data.fcols) {
        ...
        for (var i = 0, length = output.length; i < length; i++)
          data.fcols.push(
            lowercase_keys ? output[i].alias.toLowerCase() : output[i].alias
          );
      }
      ...
      while (data.fcount && data.fstatus !== 100) {
          ...
          try {
            ...
            if (custom.asObject) {
              if (item.type === SQL_BLOB)
                value = fetch_blob2_async(
                  statement,
                  value,
                  data.fcols[data.fcolumn]
                );
              data.frow[data.fcols[data.fcolumn]] = value;
            } else {
              if (item.type === SQL_BLOB)
                value = fetch_blob2_async(statement, value, data.fcolumn);
        ...
        if (!custom.asStream) data.frows.push(data.frow);
        if (custom.on) custom.on(data.frow, statement.nbrowsfetched);
        data.frow = custom.asObject ? {} : new Array(output.length);
        ...
        try {
          ...
          if (_xdrpos == data.pos) data.fop = true;
          data.r = r;
          return { error: new Error("Packet is not complete") };
        }
        statement.nbrowsfetched++;
...
Connection.prototype.executeStatement = function(
  transaction,
  statement,
  params,
  callback,
...
function fetch_blob_async(statement, id, name) {
  ...
  return function(callback) {
    ...
    statement.connection.startTransaction(ISOLATION_READ_UNCOMMITTED, function(
      err,
      transaction
    ) {
      if (err) {
...
Connection.prototype.fetchAll = function(statement, transaction, callback) {
  ...
  var loop = function(err, ret) {
    ...
    if (ret.fetched) callback(undefined, data);
    else self.fetch(statement, transaction, DEFAULT_FETCHSIZE, loop);
  };
...
Connection.prototype.svcquery = function(
  spbquery,
  resultbuffersize,
  timeout,
  callback
