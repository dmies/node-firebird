...
BlrWriter.prototype.addString = function(c, s, encoding) {
  ...
  if (len > MAX_STRING_SIZE) throw new Error("blr string is too big");
...
XdrWriter.prototype.ensure = BlrWriter.prototype.ensure = function(len) {
  ...
  while (newlen < this.pos + len) newlen *= 2;

  if (this.buffer.length >= newlen) return;
...
XdrReader.prototype.readBuffer = function(len) {
  if (!arguments.length) len = this.readInt();
